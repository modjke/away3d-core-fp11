package bmfontrenderer
{
    ...
    public class BMFont
    {
        ...
        {
            var curX:int = startX + outline;
            var curY:int = startY + outline;
            ...
            for(var curCharIdx:int = 0; curCharIdx < text.length; curCharIdx++)
            {
                ...
                curX += curGlyph.xadvance + outline;
            }
        }
		
		public function measureString(text:String):Point
		{
			var width:int = outline;
			var height:int = outline;
			for(var curCharIdx:int = 0; curCharIdx < text.length; curCharIdx++)
            {
                ...
                width += curGlyph.xadvance + outline;
				if (curGlyph.height + outline > height)
					height = curGlyph.height + outline;
            }
         ...
         * Add a bitmap sheet.
         */
        public function addSheet(id:int, bits:BitmapData, flip:Boolean = false):void
        {
            if(sheets[id] != null)
                throw new Error("Overwriting sheet!");
            sheets[id] = flip ? flipVert(bits) : bits;
        }
        ...
        public function parseFont(fontDesc:String):void
        {
            ...
            for(var i:int=0; i<fontLines.length; i++)
            {
                ...
                if(keyWord == "info")
                {
                    for (var j:int = 0; j < fontLine.length; j++)
					{
						var pair:Array = (fontLine[j] as String).split("=");
						switch (pair[0])
						{
							case "outline":
									this.outline = parseInt(pair[1]);
								break;
						}
					}
                    continue;
