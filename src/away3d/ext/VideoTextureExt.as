...
public class VideoTextureExt extends BitmapTexture
{
	public static const DRAW_FRAME:String = "drawFrame";
	
	protected var _player : IVideoPlayer;
	protected var _clippingRect : Rectangle;
	...
	private var _msPerUpdate : int;

	public function VideoTextureExt(source : String, materialWidth : uint = 256, materialHeight : uint = 256, fps:int = 24, loop : Boolean = true, autoPlay : Boolean = false, player : IVideoPlayer = null)
	{
		_msPerUpdate = int(1000 / fps);
		...
		super(new BitmapData(_materialWidth, _materialHeight, false, 0), false);
	...
	public function update() : void
	{
		...
		if (getTimer() - _lastUpdate > _msPerUpdate && _player.playing && !_player.paused) {

			_lastUpdate = getTimer();
			bitmapData.lock();
			bitmapData.fillRect(_clippingRect, 0);
			bitmapData.drawWithQuality(_player.container, null, null, null, _clippingRect, true, StageQuality.LOW);
			bitmapData.unlock();
			invalidateContent();
			
			dispatchEvent(new Event(DRAW_FRAME));
		}
